...
def handle_command(options, client, parsers_map):
    if options.create == 'detector':
        ...
        if options.raster == 'file':
            ...
            raise InvalidOptionError(parsers_map['raster'])
        if options.detector:
            i = 0
        ...
        raise InvalidOptionError(parsers_map['create'])
...
def set_parser(parser):
    ...
    # Create parsers
    create_parser = parser.add_parser('create', help="Create resources")
    ...
    create_parser.set_defaults(func=lambda a: handle_command(a, client, parsers_map))
